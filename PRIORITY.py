...
for i in range(n):
	...
	prio_time[i]=int(input("Enter the priority for Process%d: "%(i+1)))
print("Process\t Arrival_time\t Burst_time\t Priority")
for i in range(n):
	print("P%d \t\t %d \t\t %d \t\t %d" %(i+1, arr_time[i], bur_time[i], prio_time[i]))
fi_time=[0]*(n+1)
fi_time[0]=int(0)
indices=[0]*n
min_arr=arr_time[0]
...
while i<n:
	j=int(0)
	min_prio=prio_time[0]
	min_index=int(0)
	while j<n:
		while min_prio==-1:
			min_index+=1
			min_prio=prio_time[min_index]
			#endwhile
		if arr_time[j]<=min_arr and prio_time[j]<min_prio and prio_time[j]!=-1:
			min_index=j
			#endif
		j+=1
		#endwhile
